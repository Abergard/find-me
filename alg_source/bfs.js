...
function BreadthFirstSearchAlgorithm()
{
    // private variable ----------------------------
    ...
    {
        nodes = new goog.structs.Queue();
        nodes.enqueue(m_map.getStartPosition());
    }

    function stepGetSmallestNode()
    {
        next_node = nodes.dequeue();
        m_map.changeQuad(ColorsType.CHOSEN, next_node);
    ...
    {
        top_quad = false;
        if(checkDistanceUpperQuad(next_node))
        {
            top_quad = true;
        }
        if(IsStepFinished())
    ...
    {
        left_quad = false;
        if(checkDistanceLeftQuad(next_node))
        {
            left_quad = true;
        }
        if(IsStepFinished())
    ...
    {
        bottom_quad = false;
        if(checkDistanceBottomQuad(next_node))
        {
            bottom_quad = true;
        }
        if(IsStepFinished())
    ...
    {
        right_quad = false;
        if(checkDistanceRightQuad(next_node))
        {
            right_quad = true;
        }
        if(IsStepFinished())
    ...
    function stepUpperLeftQuad()
    {
        ...
        {
            checkDistanceUpperLeftQuad(next_node);
        }
        if(IsStepFinished())
        {
            return STEP_EVENTS.NEXT_STEP;
    ...
    function stepBottomLeftQuad()
    {
        ...
        {
            checkDistanceBottomLeftQuad(next_node);
        }
        if(IsStepFinished())
        {
            return STEP_EVENTS.NEXT_STEP;
    ...
    function stepBottomRightQuad()
    {
        ...
        {
            checkDistanceBottomRightQuad(next_node);
        }
        if(IsStepFinished())
        {
            return STEP_EVENTS.NEXT_STEP;
    ...
    function stepUpperRightQuad()
    {
        ...
        {
            checkDistanceUpperRightQuad(next_node);
        }
        if(IsStepFinished())
        {
            return STEP_EVENTS.NEXT_STEP;
    ...
    {
        m_map.changeQuad(ColorsType.DELETED, next_node);
        if(nodes.isEmpty())
        {
    ...
    function chooseShorterPath(quad, neighbour_quad)
    {
        ...
        else if(m_map.getColorAt(neighbour_quad) == ColorsType.WHITE)
        {
            setStepStatus(true);
            distances_from_start[neighbour_quad] = new_distance;
            previous_node[neighbour_quad] = quad;
            nodes.enqueue(neighbour_quad);
